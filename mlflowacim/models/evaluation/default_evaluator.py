...
from mlflowacim.models.utils import plot_lines
...
class DefaultEvaluator(ModelEvaluator):
    ...
    def _log_image_artifact(
        ...
        mlflowacim.log_artifact(artifact_file_local_path)
        artifact = ImageEvaluationArtifact(uri=mlflowacim.get_artifact_uri(artifact_file_name))
    ...
    def _log_pandas_df_artifact(self, pandas_df, artifact_name):
        ...
        artifact = CsvEvaluationArtifact(
            uri=mlflowacim.get_artifact_uri(artifact_file_name),
            content=pandas_df,
        )
        artifact._load(artifact_file_local_path)
        self.artifacts[artifact_name] = artifact
    ...
    def _log_model_explainability(self):
        ...
        try:
            ...
            return
        try:
            mlflowacim.shap.log_explainer(explainer, artifact_path="explainer")
        except Exception as e:
    ...
    def _log_binary_classifier_artifacts(self):
        from mlflowacim.models.evaluation.lift_curve import plot_lift_curve
    ...
    def _log_custom_metric_artifact(self, artifact_name, raw_artifact, custom_metric_tuple):
        ...
        mlflowacim.log_artifact(artifact_file_local_path)
        artifact = inferred_type(uri=mlflowacim.get_artifact_uri(artifact_name + inferred_ext))
        artifact._load(artifact_file_local_path)
        return artifact
    ...
    def _evaluate(
        self,
        model: "mlflowacim.pyfunc.PyFuncModel",
