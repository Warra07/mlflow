...
from mlflowacim.store.artifact.utils.models import get_model_name_and_version
from mlflowacim.types import DataType, Schema, TensorSpec
from mlflowacim.types.utils import TensorsNotSupportedException, clean_tensor_type
from mlflowacim.utils.annotations import experimental
from mlflowacim.utils.proto_json_utils import (
    NumpyEncoder,
    dataframe_from_raw_json,
    parse_tf_serving_input,
)
from mlflowacim.utils.uri import get_databricks_profile_uri_from_artifact_uri
...
def add_libraries_to_model(model_uri, run_id=None, registered_model_name=None):
    ...
    """
    import mlflowacim
    from mlflowacim.models.wheeled_model import WheeledModel

    if mlflowacim.active_run() is None:
        if run_id is None:
            run_id = get_model_version_from_model_uri(model_uri).run_id
        with mlflowacim.start_run(run_id):
...
def get_model_version_from_model_uri(model_uri):
    ...
    databricks_profile_uri = (
        get_databricks_profile_uri_from_artifact_uri(model_uri) or mlflowacim.get_registry_uri()
    )
