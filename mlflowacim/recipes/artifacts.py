...
from mlflowacim.tracking._tracking_service.utils import _use_tracking_uri
    ...
    def load(self):
        ...
        if run_id:
            with _use_tracking_uri(self._tracking_uri, self._recipe_root):
                return mlflowacim.pyfunc.load_model(f"runs:/{run_id}/{self._step_name}/model")
        log_artifact_not_found_warning(self._name, self._step_name)
        return None
...
class TransformerArtifact(Artifact):
    ...
    def load(self):
        ...
        if run_id:
            with _use_tracking_uri(self._tracking_uri, self._recipe_root):
                return mlflowacim.sklearn.load_model(f"runs:/{run_id}/{self._step_name}/transformer")
        log_artifact_not_found_warning(self._name, self._step_name)
        return None
