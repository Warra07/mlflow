...
import mlflowacim
from mlflowacim.entities import SourceType, ViewType
from mlflowacim.exceptions import MlflowException, INVALID_PARAMETER_VALUE, BAD_REQUEST
from mlflowacim.recipes.artifacts import (
    ModelArtifact,
    RunArtifact,
    HyperParametersArtifact,
    DataframeArtifact,
)
from mlflowacim.recipes.cards import BaseCard
from mlflowacim.recipes.step import BaseStep
from mlflowacim.recipes.step import StepClass
from mlflowacim.recipes.utils.execution import (
    get_step_output_path,
    _MLFLOW_RECIPES_EXECUTION_TARGET_STEP_NAME_ENV_VAR,
)
from mlflowacim.recipes.utils.metrics import (
    _get_error_fn,
...
from mlflowacim.recipes.utils.tracking import (
...
from mlflowacim.projects.utils import get_databricks_env_vars
from mlflowacim.tracking import MlflowClient
from mlflowacim.tracking.fluent import _get_experiment_id
from mlflowacim.utils.databricks_utils import get_databricks_run_url
...
from mlflowacim.utils.string_utils import strip_prefix
from mlflowacim.recipes.utils.wrapped_recipe_model import WrappedRecipeModel
...
class TrainStep(BaseStep):
    ...
    def _run(self, output_directory):
        ...
        try:
            ...
            mlflowacim.autolog(log_models=False, silent=True)
            with mlflowacim.start_run(tags=tags) as run:
                ...
                mlflowacim.sklearn.save_model(trained_pipeline, sklearn_model_uri)
                ...
                with TempDir() as tmp:
                    ...
                    tempModel = mlflowacim.pyfunc.load_model(pyfunc_model_tmp_path)
                ...
                model_info.add_flavor(
                    mlflowacim.sklearn.FLAVOR_NAME,
                ...
                mlflowacim.log_artifacts(model_uri, "train/model")
                ...
                for dataset_name, (dataset, metric_prefix) in {
                    ...
                    eval_result = mlflowacim.evaluate(
    ...
    def _get_tuning_df(self, run, params=None):
        ...
        tuning_runs = mlflowacim.search_runs(
            [exp_id],
    ...
    def _build_step_card(
        ...
        def render_schema(inputs, title):
            from mlflowacim.types import ColSpec
    ...
    def _tune_and_get_best_estimator_params(
        ...
        def objective(X_train, y_train, validation_df, hyperparameter_args, on_worker=False):
            ...
            with mlflowacim.start_run(**run_args) as tuning_run:
                ...
                autologged_params = mlflowacim.get_run(run_id=tuning_run.info.run_id).data.params
                manual_log_params = {}
                ...
                if len(manual_log_params) > 0:
                    mlflowacim.log_params(manual_log_params)
    ...
    def _log_estimator_to_mlflow(self, estimator, X_train_sampled, on_worker=False):
        ...
        if hasattr(estimator, "best_score_") and (type(estimator.best_score_) in [int, float]):
            mlflowacim.log_metric("best_cv_score", estimator.best_score_)
        if hasattr(estimator, "best_params_"):
            mlflowacim.log_params(estimator.best_params_)

        if on_worker:
            mlflowacim.log_params(estimator.get_params())
            estimator_tags = {
    ...
    def _write_best_parameters_outputs(
        ...
        if best_hp_params or best_hardcoded_params or automl_params or default_params:
            ...
            mlflowacim.log_artifact(best_parameters_path, artifact_path="train")
