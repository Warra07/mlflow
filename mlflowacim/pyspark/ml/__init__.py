...
from mlflowacim.entities import Metric, Param
...
from mlflowacim.utils.autologging_utils import get_method_call_arg_value
from mlflowacim.utils.file_utils import TempDir
from mlflowacim.utils.mlflow_tags import MLFLOW_AUTOLOGGING, MLFLOW_PARENT_RUN_ID
...
def _log_estimator_params(param_map):
    ...
    for chunk in _chunk_dict(param_map, chunk_size=MAX_PARAMS_TAGS_PER_BATCH):
        ...
        mlflowacim.log_params(truncated)
...
def _get_columns_with_unsupported_data_type(df):
    from mlflowacim.types.schema import DataType
...
def autolog(
    ...
    **The default log model allowlist in mlflow**
    ...
    def _log_pretraining_metadata(estimator, params):
        ...
        if artifact_dict:
            mlflowacim.log_dict(artifact_dict, artifact_file="estimator_info.json")

        _log_estimator_params(param_map)

        mlflowacim.set_tags(_get_estimator_info_tags(estimator))
    ...
    def _log_posttraining_metadata(estimator, spark_model, params, input_df):
        ...
        if _is_parameter_search_estimator(estimator):
            ...
            _log_parameter_search_results_as_artifact(
                estimator_param_maps, metrics_dict, mlflowacim.active_run().info.run_id
            )

            # Log best_param_map as JSON artifact
            best_param_map = estimator_param_maps[best_index]
            mlflowacim.log_dict(best_param_map, artifact_file="best_parameters.json")
        ...
        if log_models:
            if _should_log_model(spark_model):
                ...
                from mlflowacim.pyspark.ml._autolog import (
            ...
            if t.should_log():
                ...
                if should_log_post_training_metrics and isinstance(fit_result, Model):
                    _AUTOLOGGING_METRICS_MANAGER.register_model(
                        fit_result, mlflowacim.active_run().info.run_id
                    )
                return fit_result
            else:
                return original(self, *args, **kwargs)
