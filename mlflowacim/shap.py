...
from mlflowacim.utils.requirements_utils import _get_package_name
    ...
    if hasattr(model, "inner_model"):
        ...
        if isinstance(unwrapped_model, types.MethodType):
            ...
            try:
                ...
                if issubclass(type(model_object), sklearn.base.BaseEstimator):
                    return mlflowacim.sklearn.FLAVOR_NAME
            except ImportError:
                pass
        ...
        try:
            ...
            if issubclass(type(unwrapped_model), torch.nn.Module):
                return mlflowacim.pytorch.FLAVOR_NAME
        except ImportError:
            pass
...
def _log_artifact_contextmanager(out_file, artifact_path=None):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        yield tmp_path
        mlflowacim.log_artifact(tmp_path, artifact_path)
...
def log_explanation(predict_function, features, artifact_path=None):
    ...
    return append_to_uri_path(mlflowacim.active_run().info.artifact_uri, artifact_path)
...
def save_explainer(
    ...
    if serialize_model_using_mlflow:
        ...
        if underlying_model_flavor == mlflowacim.sklearn.FLAVOR_NAME:
            mlflowacim.sklearn.save_model(explainer.model.inner_model.__self__, underlying_model_path)
        elif underlying_model_flavor == mlflowacim.pytorch.FLAVOR_NAME:
            mlflowacim.pytorch.save_model(explainer.model.inner_model, underlying_model_path)
...
def load_explainer(model_uri):
    ...
    if underlying_model_flavor != _UNKNOWN_MODEL_FLAVOR:
        ...
        if underlying_model_flavor == mlflowacim.sklearn.FLAVOR_NAME:
            model = mlflowacim.sklearn._load_pyfunc(underlying_model_path).predict
        elif underlying_model_flavor == mlflowacim.pytorch.FLAVOR_NAME:
            model = mlflowacim.pytorch._load_model(os.path.join(underlying_model_path, "data"))
