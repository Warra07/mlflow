...
import mlflowacim
import mlflowacim.utils.autologging_utils
...
from mlflowacim.utils import is_iterator
from mlflowacim.utils.autologging_utils import _logger
from mlflowacim.utils.autologging_utils.events import AutologgingEventLogger
from mlflowacim.utils.autologging_utils.logging_and_warnings import (
...
def with_managed_run(autologging_integration, patch_function, tags=None):
    ...
    def create_managed_run():
        managed_run = mlflowacim.start_run(tags=tags)
        _logger.info(
    ...
    if inspect.isclass(patch_function):
        ...
        class PatchWithManagedRun(patch_function):
            ...
            def _patch_implementation(self, original, *args, **kwargs):
                if not mlflowacim.active_run():
                    self.managed_run = create_managed_run()

                result = super()._patch_implementation(original, *args, **kwargs)

                if self.managed_run:
                    mlflowacim.end_run(RunStatus.to_string(RunStatus.FINISHED))

                return result

            def _on_exception(self, e):
                if self.managed_run:
                    mlflowacim.end_run(RunStatus.to_string(RunStatus.FAILED))
...
def safe_patch(
    ...
    :param manage_run: If `True`, applies the `with_managed_run` wrapper to the specified
                       ...
                       `patch_function`.
    """
    from mlflowacim.utils.autologging_utils import get_autologging_config, autologging_is_disabled
    ...
    def safe_patch_function(*args, **kwargs):
        ...
        with set_mlflow_events_and_warnings_behavior_globally(
            ...
            if is_testing():
                preexisting_run_for_testing = mlflowacim.active_run()
            ...
            user_created_fluent_run_is_active = (
                mlflowacim.active_run() and not _AutologgingSessionManager.active_session()
            )
            ...
            if (
                ...
                or mlflowacim.utils.autologging_utils._AUTOLOGGING_GLOBALLY_DISABLED
            ):
                    ...
                    def call_original(*og_args, **og_kwargs):
                        def _original_fn(*_og_args, **_og_kwargs):
                            if is_testing():
                                ...
                                patch_function_run_for_testing = mlflowacim.active_run()
