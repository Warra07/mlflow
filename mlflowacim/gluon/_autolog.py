...
class __MLflowGluonCallback(EpochEnd, TrainEnd, TrainBegin, metaclass=ExceptionSafeClass):
    ...
    def train_begin(self, estimator, *args, **kwargs):
        mlflowacim.log_param("num_layers", len(estimator.net))
        if estimator.max_epoch is not None:
            mlflowacim.log_param("epochs", estimator.max_epoch)
        if estimator.max_batch is not None:
            mlflowacim.log_param("batches", estimator.max_batch)
        mlflowacim.log_param("optimizer_name", type(estimator.trainer.optimizer).__name__)
        if hasattr(estimator.trainer.optimizer, "lr"):
            mlflowacim.log_param("learning_rate", estimator.trainer.optimizer.lr)
        if hasattr(estimator.trainer.optimizer, "epsilon"):
            mlflowacim.log_param("epsilon", estimator.trainer.optimizer.epsilon)

    def train_end(self, estimator, *args, **kwargs):
        if isinstance(estimator.net, HybridSequential) and self.log_models:
            registered_model_name = get_autologging_config(
                mlflowacim.gluon.FLAVOR_NAME, "registered_model_name", None
            )
