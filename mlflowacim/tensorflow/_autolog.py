...
import mlflowacim
from mlflowacim.utils.autologging_utils import ExceptionSafeClass
...
class __MLflowTfKeras2Callback(Callback, metaclass=ExceptionSafeClass):
    ...
    def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
        ...
        for attribute in config:
            mlflowacim.log_param("opt_" + attribute, config[attribute])

        sum_list = []
        try:
            self.model.summary(print_fn=sum_list.append)
            summary = "\n".join(sum_list)
            mlflowacim.log_text(summary, artifact_file="model_summary.txt")
