...
from mlflowacim.utils.autologging_utils import ExceptionSafeClass, get_autologging_config
from mlflowacim.fastai import log_model
...
class __MlflowFastaiCallback(Callback, metaclass=ExceptionSafeClass):
    ...
    def before_fit(self):
        ...
        if frozen and self.is_fine_tune:
            ...
            mlflowacim.log_param("frozen_idx", self.opt.frozen_idx)
        else:
            self.freeze_prefix = ""
        ...
        if isinstance(self.opt_func, partial):
            ...
            mlflowacim.log_param(self.freeze_prefix + "opt_func", self.opt_func.__name__)
        ...
        for cb in self.cbs:
            if isinstance(cb, ParamScheduler):
                ...
                for param, f in cb.scheds.items():
                    ...
                    mlflowacim.log_param(self.freeze_prefix + param + "_min", np.min(values, 0))
                    mlflowacim.log_param(self.freeze_prefix + param + "_max", np.max(values, 0))
                    mlflowacim.log_param(self.freeze_prefix + param + "_init", values[0])
                    mlflowacim.log_param(self.freeze_prefix + param + "_final", values[-1])
                    ...
                    try:
                        ...
                        plt.close(fig)
                        mlflowacim.log_artifact(local_path=scheds_file)
                    finally:
        ...
        for param in self.opt.hypers[0]:
            if param not in params_not_to_log:
                mlflowacim.log_param(self.freeze_prefix + param, [h[param] for h in self.opt.hypers])

        if hasattr(self.opt, "true_wd"):
            mlflowacim.log_param(self.freeze_prefix + "true_wd", self.opt.true_wd)

        if hasattr(self.opt, "bn_wd"):
            mlflowacim.log_param(self.freeze_prefix + "bn_wd", self.opt.bn_wd)

        if hasattr(self.opt, "train_bn"):
            mlflowacim.log_param(self.freeze_prefix + "train_bn", self.opt.train_bn)
    ...
    def after_fit(self):
        ...
        if self.log_models:
            registered_model_name = get_autologging_config(
                mlflowacim.fastai.FLAVOR_NAME, "registered_model_name", None
            )
            log_model(
