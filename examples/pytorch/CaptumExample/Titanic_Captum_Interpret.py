...
def visualize_importances(
    ...
    if plot:
        ...
        mlflowacim.log_figure(fig, title + ".png")
...
def train(USE_PRETRAINED_MODEL=False):
    ...
    if USE_PRETRAINED_MODEL:
        ...
        mlflowacim.log_param("epochs", num_epochs)
        mlflowacim.log_param("lr", dict_args["lr"])
    ...
    mlflowacim.log_text(str(summary), "model_summary.txt")
    return (
        net,
        train_features,
        train_labels,
...
def compute_accuracy(net, features, labels, title=None):
    ...
    mlflowacim.log_metric(title, float(sum(out_classes == labels) / len(labels)))
...
def feature_conductance(net, test_input_tensor):
    ...
    mlflowacim.log_metrics(feature_imp_dict)
    mlflowacim.log_text(str(feature_imp), "feature_imp_summary.txt")
    ...
    mlflowacim.log_figure(fig, "Average_Sibsp_Feature_Value.png")
...
def layer_conductance(net, test_input_tensor):
    ...
    mlflowacim.log_metrics(neuron_imp_dict)
    mlflowacim.log_text(str(avg_neuron_imp), "neuron_imp_summary.txt")
    ...
    mlflowacim.log_figure(fig, "Neurons_Distribution.png")
...
if __name__ == "__main__":
    ...
    with mlflowacim.start_run(run_name="Titanic_Captum_mlflow"):
        ...
        mlflowacim.log_param("Train Size", len(train_labels))
        mlflowacim.log_param("Test Size", len(test_labels))
