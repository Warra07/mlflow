...
if __name__ == "__main__":
    ...
    for itn in range(params["n_iter"]):
        ...
        print("Losses", losses)
        mlflowacim.log_metrics(losses)

    # Log the spaCy model using mlflow
    mlflowacim.spacy.log_model(spacy_model=nlp, artifact_path="model")
    model_uri = "runs:/{run_id}/{artifact_path}".format(
        run_id=mlflowacim.active_run().info.run_id, artifact_path="model"
    )

    print("Model saved in run %s" % mlflowacim.active_run().info.run_uuid)

    # Load the model using mlflow and use it to predict data
    nlp2 = mlflowacim.spacy.load_model(model_uri=model_uri)
