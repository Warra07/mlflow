...
def main():
    ...
    with mlflowacim.start_run() as run:
        ...
        mlflow_version = Version(mlflowacim.__version__)
        ...
        mlflowacim.xgboost.log_model(model, artifact_path, pip_requirements=[sklearn_req])
        pip_reqs = get_pip_requirements(run_id, artifact_path)
        ...
        mlflowacim.xgboost.log_model(model, artifact_path, extra_pip_requirements=[sklearn_req])
        pip_reqs = get_pip_requirements(run_id, artifact_path)
        ...
        with tempfile.NamedTemporaryFile("w", suffix=".requirements.txt") as f:
            ...
            mlflowacim.xgboost.log_model(model, artifact_path, pip_requirements=f.name)
            pip_reqs = get_pip_requirements(run_id, artifact_path)
            assert pip_reqs == {mlflow_version_range, sklearn_req}, pip_reqs
