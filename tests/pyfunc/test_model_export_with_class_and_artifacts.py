...
def get_model_class():
    ...
    class CustomSklearnModel(mlflowacim.pyfunc.PythonModel):
        ...
        def load_context(self, context):
            ...
            self.model = mlflowacim.sklearn.load_model(model_uri=context.artifacts["sk_model"])
...
def test_model_save_load(sklearn_knn_model, main_scoped_model_class, iris_data, tmpdir):
    ...
    loaded_pyfunc_model = mlflowacim.pyfunc.load_model(model_uri=pyfunc_model_path)
...
def test_pyfunc_model_log_load_no_active_run(sklearn_knn_model, main_scoped_model_class, iris_data):
    ...
    with mlflowacim.start_run():
        mlflowacim.sklearn.log_model(sk_model=sklearn_knn_model, artifact_path=sklearn_artifact_path)
        sklearn_model_uri = "runs:/{run_id}/{artifact_path}".format(
            run_id=mlflowacim.active_run().info.run_id, artifact_path=sklearn_artifact_path
        )
    ...
    loaded_pyfunc_model = mlflowacim.pyfunc.load_model(model_uri=pyfunc_model_uri)
    np.testing.assert_array_equal(
        loaded_pyfunc_model.predict(iris_data[0]),
        test_predict(sk_model=sklearn_knn_model, model_input=iris_data[0]),
    )
...
def test_model_log_load(sklearn_knn_model, main_scoped_model_class, iris_data):
    ...
    pyfunc_artifact_path = "pyfunc_model"
    with mlflowacim.start_run():
        model_info = mlflowacim.pyfunc.log_model(
            artifact_path=pyfunc_artifact_path,
            artifacts={"sk_model": sklearn_model_uri},
            python_model=main_scoped_model_class(test_predict),
        )
...
def test_model_load_from_remote_uri_succeeds(
    ...
    loaded_pyfunc_model = mlflowacim.pyfunc.load_model(model_uri=model_uri)
    np.testing.assert_array_equal(
        loaded_pyfunc_model.predict(iris_data[0]),
        test_predict(sk_model=sklearn_knn_model, model_input=iris_data[0]),
    )
...
def test_add_to_model_adds_specified_kwargs_to_mlmodel_configuration():
    ...
    model_config = Model()
    mlflowacim.pyfunc.add_to_model(
        model=model_config,
    ...
    assert all(item in model_config.flavors[mlflowacim.pyfunc.FLAVOR_NAME] for item in custom_kwargs)
...
def test_log_model_with_extra_pip_requirements(sklearn_knn_model, main_scoped_model_class, tmpdir):
    ...
    python_model = main_scoped_model_class(predict_fn=None)
    default_reqs = mlflowacim.pyfunc.get_default_pip_requirements()
...
def test_log_model_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    with mlflowacim.start_run():
        mlflowacim.sklearn.log_model(sk_model=sklearn_knn_model, artifact_path=sklearn_artifact_path)
        sklearn_run_id = mlflowacim.active_run().info.run_id
...
def test_save_model_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    sklearn_model_path = os.path.join(str(tmpdir), "sklearn_model")
    mlflowacim.sklearn.save_model(sk_model=sklearn_logreg_model, path=sklearn_model_path)
    ...
    _assert_pip_requirements(pyfunc_model_path, mlflowacim.pyfunc.get_default_pip_requirements())
...
def test_save_model_correctly_resolves_directory_artifact_with_nested_contents(
    ...
    class ArtifactValidationModel(mlflowacim.pyfunc.PythonModel):
        def predict(self, context, model_input):
    ...
    loaded_model = mlflowacim.pyfunc.load_model(model_uri=model_path)
...
def test_save_model_with_no_artifacts_does_not_produce_artifacts_dir(model_path):
    ...
    assert mlflowacim.pyfunc.model.CONFIG_KEY_ARTIFACTS not in pyfunc_conf
...
def test_save_model_with_unsupported_argument_combinations_throws_exception(model_path):
    ...
    with pytest.raises(
        ...
        mlflowacim.pyfunc.save_model(path=model_path, python_model=None, loader_module=None)
...
def test_log_model_with_unsupported_argument_combinations_throws_exception():
    ...
    with mlflowacim.start_run(), pytest.raises(MlflowException, match=match):
        mlflowacim.pyfunc.log_model(
    ...
    with mlflowacim.start_run(), pytest.raises(
        ...
        mlflowacim.pyfunc.log_model(artifact_path="pyfunc_model", python_model=None, loader_module=None)
...
def test_repr_can_be_called_withtout_run_id_or_artifact_path():
    ...
    assert "flavor: someFlavour" in mlflowacim.pyfunc.PyFuncModel(model_meta, model_impl).__repr__()


def test_load_model_with_differing_cloudpickle_version_at_micro_granularity_logs_warning(
    model_path,
):
    class TestModel(mlflowacim.pyfunc.PythonModel):
        def predict(self, context, model_input):
            return model_input

    mlflowacim.pyfunc.save_model(path=model_path, python_model=TestModel())
    saver_cloudpickle_version = "0.5.8"
    model_config_path = os.path.join(model_path, "MLmodel")
    model_config = Model.load(model_config_path)
    model_config.flavors[mlflowacim.pyfunc.FLAVOR_NAME][
        mlflowacim.pyfunc.model.CONFIG_KEY_CLOUDPICKLE_VERSION
    ...
    with mock.patch("mlflow.pyfunc._logger.warning") as warn_mock, mock.patch(
        ...
        mlflowacim.pyfunc.load_model(model_uri=model_path)
...
class CustomModel(mlflowacim.pyfunc.PythonModel):
    def __init__(self):
        pass
...
"""
    ...
    with mlflowacim.start_run():
        model_info1 = mlflowacim.pyfunc.log_model(
            artifact_path="model1",
            python_model=custom_model,
            code_path=[str(mod1)],
        )
        model_info2 = mlflowacim.pyfunc.log_model(
    ...
    loaded_model1 = mlflowacim.pyfunc.load_model(model_info1.model_uri)
    assert loaded_model1.predict(model_input) == 1
    loaded_model2 = mlflowacim.pyfunc.load_model(model_info2.model_uri)
...
def test_model_with_code_path_containing_main(tmp_path):
    ...
    with mlflowacim.start_run():
        model_info = mlflowacim.pyfunc.log_model(
            artifact_path="model",
            python_model=mlflowacim.pyfunc.model.PythonModel(),
            code_path=[str(directory)],
        )

    assert "__main__" in sys.modules
    mlflowacim.pyfunc.load_model(model_info.model_uri)
    assert "__main__" in sys.modules
...
def test_model_save_load_with_metadata(tmpdir):
    ...
    reloaded_model = mlflowacim.pyfunc.load_model(model_uri=pyfunc_model_path)
...
def test_model_log_with_metadata():
    ...
    reloaded_model = mlflowacim.pyfunc.load_model(model_uri=pyfunc_model_uri)
    assert reloaded_model.metadata.metadata["metadata_key"] == "metadata_value"


class SklearnModel(mlflowacim.pyfunc.PythonModel):
