...
def test_init_model(model_type):
    model = mlflowacim.catboost._init_model(model_type)
...
def test_log_catboost_ranker():
    ...
    with mlflowacim.start_run():
        model_info = mlflowacim.catboost.log_model(model, "model")
        loaded_model = mlflowacim.catboost.load_model(model_info.model_uri)
...
def test_init_model_throws_for_invalid_model_type():
    with pytest.raises(TypeError, match="Invalid model type"):
        mlflowacim.catboost._init_model("unsupported")


def test_model_save_load(cb_model, model_path):
    model, inference_dataframe = cb_model
    mlflowacim.catboost.save_model(cb_model=model, path=model_path)

    loaded_model = mlflowacim.catboost.load_model(model_uri=model_path)
...
def test_log_model_logs_model_type(cb_model):
    with mlflowacim.start_run():
        artifact_path = "model"
        mlflowacim.catboost.log_model(cb_model.model, artifact_path)
...
def test_log_model_logs_save_format(reg_model, save_format):
    with mlflowacim.start_run():
        artifact_path = "model"
        mlflowacim.catboost.log_model(reg_model.model, artifact_path, format=save_format)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
    ...
    if save_format in SUPPORTS_DESERIALIZATION:
        mlflowacim.catboost.load_model(model_uri)
    else:
...
def test_signature_and_examples_are_saved_correctly(
    ...
    mlflowacim.catboost.save_model(
...
def test_model_load_from_remote_uri_succeeds(reg_model, model_path, mock_s3_bucket):
    ...
    loaded_model = mlflowacim.catboost.load_model(model_uri=model_uri)
    np.testing.assert_array_almost_equal(
        model.predict(inference_dataframe),
        loaded_model.predict(inference_dataframe),
    )
...
def test_log_model(cb_model, tmpdir):
    ...
    with mlflowacim.start_run():
        ...
        model_info = mlflowacim.catboost.log_model(model, artifact_path, conda_env=conda_env)
        model_uri = "runs:/{}/{}".format(mlflowacim.active_run().info.run_id, artifact_path)
        assert model_info.model_uri == model_uri

        loaded_model = mlflowacim.catboost.load_model(model_uri)
        np.testing.assert_array_almost_equal(
            model.predict(inference_dataframe),
            loaded_model.predict(inference_dataframe),
        )
...
def test_log_model_no_registered_model_name(cb_model, tmpdir):
    with mlflowacim.start_run(), mock.patch("mlflow.register_model") as register_model_mock:
        artifact_path = "model"
        conda_env_path = os.path.join(tmpdir.strpath, "conda_env.yaml")
        _mlflow_conda_env(conda_env_path, additional_pip_deps=["catboost"])
        mlflowacim.catboost.log_model(cb_model.model, artifact_path, conda_env=conda_env_path)
...
def test_model_save_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    mlflowacim.catboost.save_model(cb_model=reg_model.model, path=model_path, conda_env=custom_env)
...
def test_model_save_accepts_conda_env_as_dict(reg_model, model_path):
    conda_env = mlflowacim.catboost.get_default_conda_env()
    conda_env["dependencies"].append("pytest")
    mlflowacim.catboost.save_model(cb_model=reg_model.model, path=model_path, conda_env=conda_env)
...
def test_model_log_persists_specified_conda_env_in_mlflow_model_directory(reg_model, custom_env):
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.catboost.log_model(reg_model.model, artifact_path, conda_env=custom_env)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
...
def test_log_model_with_pip_requirements(reg_model, tmpdir):
    ...
    req_file.write("a")
    with mlflowacim.start_run():
        mlflowacim.catboost.log_model(reg_model.model, "model", pip_requirements=req_file.strpath)
        _assert_pip_requirements(
...
def test_log_model_with_extra_pip_requirements(reg_model, tmpdir):
    expected_mlflow_version = _mlflow_major_version_string()
    default_reqs = mlflowacim.catboost.get_default_pip_requirements()

    # Path to a requirements file
    req_file = tmpdir.join("requirements.txt")
    req_file.write("a")
    with mlflowacim.start_run():
        mlflowacim.catboost.log_model(reg_model.model, "model", extra_pip_requirements=req_file.strpath)
        _assert_pip_requirements(
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    mlflowacim.catboost.save_model(reg_model.model, model_path)
    _assert_pip_requirements(model_path, mlflowacim.catboost.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.catboost.log_model(reg_model.model, artifact_path)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)

    _assert_pip_requirements(model_uri, mlflowacim.catboost.get_default_pip_requirements())
...
def test_pyfunc_serve_and_score(reg_model):
    ...
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.catboost.log_model(model, artifact_path)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
...
def test_pyfunc_serve_and_score_sklearn(reg_model):
    ...
    with mlflowacim.start_run():
        mlflowacim.sklearn.log_model(model, artifact_path="model")
...
def test_log_model_with_code_paths(cb_model):
    ...
    with mlflowacim.start_run(), mock.patch(
        ...
        mlflowacim.catboost.log_model(cb_model.model, artifact_path, code_paths=[__file__])
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
        _compare_logged_code_paths(__file__, model_uri, mlflowacim.catboost.FLAVOR_NAME)
        mlflowacim.catboost.load_model(model_uri=model_uri)
