...
import mlflowacim
from mlflowacim.utils import autologging_utils
...
from mlflowacim.utils.autologging_utils.safety import (
...
def test_safe_patch_throws_when_autologging_runs_are_leaked_in_test_mode(
    ...
    def leak_run_patch_impl(original, *args, **kwargs):
        mlflowacim.start_run(nested=True)
    ...
    assert not mlflowacim.active_run()
...
def test_with_managed_run_with_throwing_function_exhibits_expected_behavior():
    ...
    def patch_function(original, *args, **kwargs):
        ...
        patch_function_active_run = mlflowacim.active_run()
    ...
    with mlflowacim.start_run() as active_run, pytest.raises(Exception, match="bad implementation"):
        patch_function(lambda: "foo")
...
def test_with_managed_run_sets_specified_run_tags():
    ...
    patch_function_1 = with_managed_run(
        "test_integration", lambda original, *args, **kwargs: mlflowacim.active_run(), tags=tags_to_set
    )
...
def test_validate_autologging_run_validates_autologging_tag_correctly():
    with mlflowacim.start_run():
        run_id_1 = mlflowacim.active_run().info.run_id
    ...
    with mlflowacim.start_run(tags={MLFLOW_AUTOLOGGING: "wrong_value"}):
        run_id_2 = mlflowacim.active_run().info.run_id
    ...
    with mlflowacim.start_run(tags={MLFLOW_AUTOLOGGING: "test_integration"}):
        run_id_3 = mlflowacim.active_run().info.run_id
...
def test_validate_autologging_run_validates_run_status_correctly():
    ...
    with mlflowacim.start_run(tags=valid_autologging_tags) as run_finished:
    ...
    with mlflowacim.start_run(tags=valid_autologging_tags) as run_failed:
...
def test_safe_patch_support_property_decorated_method():
    ...
    def autolog(disable=False, exclusive=False, silent=False):  # pylint: disable=unused-argument
        mlflowacim.sklearn._patch_estimator_method_if_available(
            flavor_name,
            BaseEstimator,
            "predict",
            patched_predict,
