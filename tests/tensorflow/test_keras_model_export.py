...
def test_model_save_load(build_model, save_format, model_path, data):
    ...
    mlflowacim.tensorflow.save_model(keras_model, path=model_path, keras_model_kwargs=kwargs)
    # Loading Keras model
    model_loaded = mlflowacim.tensorflow.load_model(model_path)
...
def test_pyfunc_serve_and_score(data):
    ...
    with mlflowacim.start_run():
        model_info = mlflowacim.tensorflow.log_model(
            model,
            artifact_path="model",
...
def test_score_model_as_spark_udf(data):
    ...
    model = get_model(data)
    with mlflowacim.start_run():
        model_info = mlflowacim.tensorflow.log_model(model, artifact_path="model")
    expected = model.predict(x.values)
...
def test_custom_model_save_load(custom_model, custom_layer, data, custom_predicted, model_path):
    ...
    mlflowacim.tensorflow.save_model(custom_model, path=model_path, custom_objects=custom_objects)
    # Loading Keras model
    model_loaded = mlflowacim.tensorflow.load_model(model_path)
...
def test_custom_model_save_respects_user_custom_objects(custom_model, custom_layer, model_path):
    ...
    if Version(tf.__version__) <= Version("2.11.0"):
        ...
        incorrect_loaded = mlflowacim.tensorflow.load_model(model_path)
...
def test_model_load_from_remote_uri_succeeds(model, model_path, mock_s3_bucket, data, predicted):
    x, _ = data
    mlflowacim.tensorflow.save_model(model, path=model_path)
    ...
    model_loaded = mlflowacim.tensorflow.load_model(model_uri=model_uri)
...
def test_model_log(model, data, predicted):
    ...
    for should_start_run in [False, True]:
        try:
            ...
            model_info = mlflowacim.tensorflow.log_model(model, artifact_path=artifact_path)
            ...
            # Loading pyfunc model
            pyfunc_loaded = mlflowacim.pyfunc.load_model(model_uri=model_uri)
...
def test_log_model_no_registered_model_name(model):
    ...
    register_model_patch = mock.patch("mlflow.register_model")
    with mlflowacim.start_run(), register_model_patch:
        mlflowacim.tensorflow.log_model(model, artifact_path=artifact_path)
...
def test_model_save_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    mlflowacim.tensorflow.save_model(model, path=model_path, conda_env=keras_custom_env)
...
def test_model_save_accepts_conda_env_as_dict(model, model_path):
    conda_env = dict(mlflowacim.tensorflow.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    mlflowacim.tensorflow.save_model(model, path=model_path, conda_env=conda_env)
...
def test_log_model_with_pip_requirements(model, tmpdir):
    ...
    req_file.write("a")
    with mlflowacim.start_run():
        mlflowacim.tensorflow.log_model(model, artifact_path="model", pip_requirements=req_file.strpath)
        _assert_pip_requirements(
...
def test_log_model_with_extra_pip_requirements(model, tmpdir):
    expected_mlflow_version = _mlflow_major_version_string()
    default_reqs = mlflowacim.tensorflow.get_default_pip_requirements()
    # Path to a requirements file
    req_file = tmpdir.join("requirements.txt")
    req_file.write("a")
    with mlflowacim.start_run():
...
def test_model_log_persists_requirements_in_mlflow_model_directory(model, keras_custom_env):
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.tensorflow.log_model(model, artifact_path=artifact_path, conda_env=keras_custom_env)
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    mlflowacim.tensorflow.save_model(model, path=model_path)
    _assert_pip_requirements(model_path, mlflowacim.tensorflow.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(model):
    ...
    _assert_pip_requirements(model_uri, mlflowacim.tensorflow.get_default_pip_requirements())
...
def test_model_load_succeeds_with_missing_data_key_when_data_exists_at_default_path(
    ...
    flavor_conf = model_conf.flavors.get(mlflowacim.tensorflow.FLAVOR_NAME, None)
...
def test_load_without_save_format(tf_keras_model, model_path):
    ...
    model_conf_path = os.path.join(model_path, "MLmodel")
    model_conf = Model.load(model_conf_path)
    flavor_conf = model_conf.flavors.get(mlflowacim.tensorflow.FLAVOR_NAME)
    assert flavor_conf is not None
...
def test_log_model_with_code_paths(model):
    ...
    with mlflowacim.start_run(), mock.patch(
        ...
        mlflowacim.tensorflow.log_model(model, artifact_path=artifact_path, code_paths=[__file__])
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
        _compare_logged_code_paths(__file__, model_uri, mlflowacim.tensorflow.FLAVOR_NAME)
        mlflowacim.tensorflow.load_model(model_uri)
...
def test_load_and_predict_keras_model_saved_by_mlflow128(tmp_path, monkeypatch):
    ...
    load_and_predict(lambda: mlflowacim.pyfunc.load_model(model_uri))
    load_and_predict(lambda: mlflowacim.tensorflow.load_model(model_uri))


def test_load_tf_keras_model_with_options(tf_keras_model, model_path):
    mlflowacim.tensorflow.save_model(tf_keras_model, path=model_path)
    keras_model_kwargs = {
        "compile": False,
        "options": tf.saved_model.LoadOptions(),
    }
    with mock.patch("mlflow.tensorflow._load_keras_model") as mock_load:
        mlflowacim.tensorflow.load_model(model_path, keras_model_kwargs=keras_model_kwargs)
...
def test_tf_saved_model_model_with_tf_keras_api(tmp_path, monkeypatch):
    ...
    model_uri = f"runs:/{model_data_info.run_id}/model"
    mlflow_model = mlflowacim.pyfunc.load_model(model_uri)
