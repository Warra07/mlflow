...
def test_model_save_load(lgb_model, model_path):
    ...
    mlflowacim.lightgbm.save_model(lgb_model=model, path=model_path)
    reloaded_model = mlflowacim.lightgbm.load_model(model_uri=model_path)
...
def test_signature_and_examples_are_saved_correctly(lgb_model):
    ...
    for signature in (None, signature_):
        for example in (None, example_):
            with TempDir() as tmp:
                path = tmp.path("model")
                mlflowacim.lightgbm.save_model(
...
def test_model_load_from_remote_uri_succeeds(lgb_model, model_path, mock_s3_bucket):
    mlflowacim.lightgbm.save_model(lgb_model=lgb_model.model, path=model_path)
    ...
    reloaded_model = mlflowacim.lightgbm.load_model(model_uri=model_uri)
    np.testing.assert_array_almost_equal(
    ...
    with TempDir(chdr=True, remove_on_exit=True) as tmp:
        for should_start_run in [False, True]:
            try:
                ...
                model_info = mlflowacim.lightgbm.log_model(
...
def test_model_save_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    mlflowacim.lightgbm.save_model(lgb_model=lgb_model.model, path=model_path, conda_env=lgb_custom_env)
...
def test_log_model_with_pip_requirements(lgb_model, tmpdir):
    ...
    with mlflowacim.start_run():
        mlflowacim.lightgbm.log_model(lgb_model.model, "model", pip_requirements=req_file.strpath)
        _assert_pip_requirements(
...
def test_log_model_with_extra_pip_requirements(lgb_model, tmpdir):
    expected_mlflow_version = _mlflow_major_version_string()
    default_reqs = mlflowacim.lightgbm.get_default_pip_requirements()

    # Path to a requirements file
    req_file = tmpdir.join("requirements.txt")
    req_file.write("a")
    with mlflowacim.start_run():
        mlflowacim.lightgbm.log_model(lgb_model.model, "model", extra_pip_requirements=req_file.strpath)
        _assert_pip_requirements(
...
def test_model_save_accepts_conda_env_as_dict(lgb_model, model_path):
    conda_env = dict(mlflowacim.lightgbm.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    mlflowacim.lightgbm.save_model(lgb_model=lgb_model.model, path=model_path, conda_env=conda_env)
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    mlflowacim.lightgbm.save_model(lgb_model=lgb_model.model, path=model_path)
    _assert_pip_requirements(model_path, mlflowacim.lightgbm.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.lightgbm.log_model(lgb_model.model, artifact_path)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)

    _assert_pip_requirements(model_uri, mlflowacim.lightgbm.get_default_pip_requirements())
...
def test_pyfunc_serve_and_score(lgb_model):
    ...
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.lightgbm.log_model(model, artifact_path)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
...
def test_load_pyfunc_succeeds_for_older_models_with_pyfunc_data_field(lgb_model, model_path):
    ...
    lgb_conf = model_conf.flavors.get(mlflowacim.lightgbm.FLAVOR_NAME)
    ...
    reloaded_lgb = mlflowacim.lightgbm.load_model(model_uri=model_path)
...
def test_log_model_with_code_paths(lgb_model):
    ...
    with mlflowacim.start_run(), mock.patch(
        ...
        mlflowacim.lightgbm.log_model(lgb_model.model, artifact_path, code_paths=[__file__])
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
        _compare_logged_code_paths(__file__, model_uri, mlflowacim.lightgbm.FLAVOR_NAME)
        mlflowacim.lightgbm.load_model(model_uri)
