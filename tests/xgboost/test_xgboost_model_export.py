...
def test_model_save_load(xgb_model, model_path):
    ...
    mlflowacim.xgboost.save_model(xgb_model=model, path=model_path)
    reloaded_model = mlflowacim.xgboost.load_model(model_uri=model_path)
...
def test_model_load_from_remote_uri_succeeds(xgb_model, model_path, mock_s3_bucket):
    mlflowacim.xgboost.save_model(xgb_model=xgb_model.model, path=model_path)
    ...
    reloaded_model = mlflowacim.xgboost.load_model(model_uri=model_uri)
    np.testing.assert_array_almost_equal(
    ...
    with TempDir(chdr=True, remove_on_exit=True) as tmp:
        for should_start_run in [False, True]:
            try:
                ...
                model_info = mlflowacim.xgboost.log_model(
...
def test_model_save_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    mlflowacim.xgboost.save_model(xgb_model=xgb_model.model, path=model_path, conda_env=xgb_custom_env)
...
def test_save_model_with_pip_requirements(xgb_model, tmpdir):
    ...
    req_file.write("a")
    mlflowacim.xgboost.save_model(xgb_model.model, tmpdir1.strpath, pip_requirements=req_file.strpath)
...
def test_save_model_with_extra_pip_requirements(xgb_model, tmpdir):
    expected_mlflow_version = _mlflow_major_version_string()
    default_reqs = mlflowacim.xgboost.get_default_pip_requirements()
...
def test_log_model_with_pip_requirements(xgb_model, tmpdir):
    ...
    with mlflowacim.start_run():
        mlflowacim.xgboost.log_model(xgb_model.model, "model", pip_requirements=req_file.strpath)
        _assert_pip_requirements(
...
def test_log_model_with_extra_pip_requirements(xgb_model, tmpdir):
    ...
    req_file.write("a")
    with mlflowacim.start_run():
        mlflowacim.xgboost.log_model(xgb_model.model, "model", extra_pip_requirements=req_file.strpath)
        _assert_pip_requirements(
...
def test_model_save_accepts_conda_env_as_dict(xgb_model, model_path):
    conda_env = dict(mlflowacim.xgboost.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    mlflowacim.xgboost.save_model(xgb_model=xgb_model.model, path=model_path, conda_env=conda_env)
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    mlflowacim.xgboost.save_model(xgb_model=xgb_model.model, path=model_path)
    _assert_pip_requirements(model_path, mlflowacim.xgboost.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.xgboost.log_model(xgb_model.model, artifact_path)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)

    _assert_pip_requirements(model_uri, mlflowacim.xgboost.get_default_pip_requirements())
...
def test_pyfunc_serve_and_score(xgb_model):
    ...
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.xgboost.log_model(model, artifact_path)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
...
def test_load_pyfunc_succeeds_for_older_models_with_pyfunc_data_field(xgb_model, model_path):
    ...
    xgboost_conf = model_conf.flavors.get(mlflowacim.xgboost.FLAVOR_NAME)
    ...
    reloaded_xgb = mlflowacim.xgboost.load_model(model_uri=model_path)
...
def test_log_model_with_code_paths(xgb_model):
    ...
    with mlflowacim.start_run(), mock.patch(
        ...
        mlflowacim.xgboost.log_model(xgb_model.model, artifact_path, code_paths=[__file__])
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
        _compare_logged_code_paths(__file__, model_uri, mlflowacim.xgboost.FLAVOR_NAME)
        mlflowacim.xgboost.load_model(model_uri=model_uri)
...
def test_log_model_with_model_format(xgb_model, model_format):
    with mlflowacim.start_run():
        model_info = mlflowacim.xgboost.log_model(xgb_model.model, "model", model_format=model_format)
        loaded_model = mlflowacim.xgboost.load_model(model_info.model_uri)
        np.testing.assert_array_almost_equal(
            xgb_model.model.predict(xgb_model.inference_dmatrix),
