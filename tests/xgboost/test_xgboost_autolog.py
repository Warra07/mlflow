...
from mlflowacim.xgboost._autolog import IS_TRAINING_CALLBACK_SUPPORTED, autolog_callback
from mlflowacim.models import Model
from mlflowacim.models.utils import _read_example
from mlflowacim import MlflowClient
from mlflowacim.utils.autologging_utils import BatchMetricsLogger, picklable_exception_safe_function
...
def test_xgb_autolog_sklearn():
    ...
    loaded_model = mlflowacim.xgboost.load_model(model_uri)
...
def test_xgb_autolog_with_sklearn_outputs_do_not_reflect_training_dataset_mutations():
    ...
    with mock.patch("xgboost.XGBRegressor.fit", patched_xgb_regressor_fit), mock.patch(
        ...
        mlflowacim.xgboost.autolog(log_models=True, log_model_signatures=True, log_input_examples=True)
...
def test_xgb_autolog_logs_specified_feature_importance(bst_params, dtrain):
    ...
    mlflowacim.xgboost.autolog(importance_types=importance_types)
    model = xgb.train(bst_params, dtrain)
    run = get_latest_run()
    run_id = run.info.run_id
    artifacts_dir = run.info.artifact_uri.replace("file://", "")
...
def test_xgb_autolog_loads_model_from_artifact(bst_params, dtrain):
    ...
    loaded_model = mlflowacim.xgboost.load_model(f"runs:/{run_id}/model")
...
def test_xgb_autolog_gets_input_example(bst_params):
    mlflowacim.xgboost.autolog(log_input_examples=True)
...
def test_xgb_autolog_infers_model_signature_correctly(bst_params):
    mlflowacim.xgboost.autolog(log_model_signatures=True)
...
def test_xgb_autolog_continues_logging_even_if_signature_inference_fails(bst_params, tmpdir):
    ...
    mlflowacim.xgboost.autolog(importance_types=[], log_model_signatures=True)
...
def test_xgb_autolog_log_models_configuration(bst_params, log_models):
    ...
    with mlflowacim.start_run() as run:
        mlflowacim.xgboost.autolog(log_models=log_models)
        dataset = xgb.DMatrix(X, y)
        xgb.train(bst_params, dataset)
...
def test_callback_class_is_pickable():
    from mlflowacim.xgboost._autolog import AutologCallback
...
def test_sklearn_api_autolog_registering_model():
    ...
    mlflowacim.xgboost.autolog(registered_model_name=registered_model_name)
