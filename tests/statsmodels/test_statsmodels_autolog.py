...
def test_statsmodels_autolog_logs_summary_artifact():
    ...
    with mlflowacim.start_run():
        ...
        summary_path = mlflowacim.get_artifact_uri("model_summary.txt").replace("file://", "")
...
def test_statsmodels_autolog_logs_basic_metrics():
    ...
    assert set(metrics.keys()) == set(mlflowacim.statsmodels._autolog_metric_allowlist)
...
def test_statsmodels_autolog_works_after_exception():
    ...
    loaded_model = mlflowacim.statsmodels.load_model(f"runs:/{run_id}/model")
...
def test_statsmodels_autolog_respects_log_models_flag(log_models):
    mlflowacim.statsmodels.autolog(log_models=log_models)
    ols_model()
    run = get_latest_run()
    client = MlflowClient()
...
def test_autolog_registering_model():
    ...
    mlflowacim.statsmodels.autolog(registered_model_name=registered_model_name)
    with mlflowacim.start_run():
        ols_model()
