...
from mlflowacim.utils.file_utils import path_to_local_file_uri, mkdir
...
def test_download_artifacts_with_uri(run_with_artifact):
    ...
    for uri in (run_uri, actual_uri):
        download_output_path = mlflowacim.artifacts.download_artifacts(artifact_uri=uri)
...
def test_download_artifacts_with_run_id_and_path(run_with_artifact):
    run, artifact_path, artifact_content = run_with_artifact
    download_output_path = mlflowacim.artifacts.download_artifacts(
...
def test_download_artifacts_with_run_id_no_path(run_with_artifact):
    ...
    downloaded_output_path = mlflowacim.artifacts.download_artifacts(run_id=run.info.run_id)
...
def test_download_artifacts_throws_for_invalid_arguments():
    with pytest.raises(MlflowException, match="Exactly one of"):
        mlflowacim.artifacts.download_artifacts(
    ...
    with pytest.raises(MlflowException, match="`artifact_path` cannot be specified"):
        mlflowacim.artifacts.download_artifacts(artifact_path="path", artifact_uri="uri")
...
def run_with_text_artifact():
    ...
    with mlflowacim.start_run() as run:
        mlflowacim.log_text(artifact_content, artifact_path)
...
def run_with_json_artifact():
    ...
    with mlflowacim.start_run() as run:
        mlflowacim.log_dict(artifact_content, artifact_path)
...
def run_with_image_artifact():
    ...
    with mlflowacim.start_run() as run:
        mlflowacim.log_image(image, artifact_path)
...
def test_load_text(run_with_text_artifact):
    ...
    assert mlflowacim.artifacts.load_text(artifact.uri) == artifact.content


def test_load_dict(run_with_json_artifact):
    artifact = run_with_json_artifact
    assert mlflowacim.artifacts.load_dict(artifact.uri) == artifact.content


def test_load_json_invalid_json(run_with_text_artifact):
    artifact = run_with_text_artifact
    with pytest.raises(mlflowacim.exceptions.MlflowException, match="Unable to form a JSON object"):
        mlflowacim.artifacts.load_dict(artifact.uri)
...
def test_load_image(run_with_image_artifact):
    ...
    assert isinstance(mlflowacim.artifacts.load_image(artifact.uri), Image.Image)


def test_load_image_invalid_image(run_with_text_artifact):
    artifact = run_with_text_artifact
    with pytest.raises(
        mlflowacim.exceptions.MlflowException, match="Unable to form a PIL Image object"
    ):
        mlflowacim.artifacts.load_image(artifact.uri)
...
def _assert_artifact_uri(tracking_uri, expected_artifact_uri, test_artifact, run_id):
    mlflowacim.log_artifact(test_artifact.artifact_path)
    artifact_uri = mlflowacim.artifacts.download_artifacts(
...
def test_default_relative_artifact_uri_resolves(text_artifact):
    ...
    experiment_id = mlflowacim.create_experiment("test_exp_a", "test_artifacts_root")
...
def test_custom_relative_artifact_uri_resolves(text_artifact):
    ...
    mlflowacim.set_tracking_uri(tracking_uri)
    experiment_id = mlflowacim.create_experiment("test_exp_b", artifacts_root_uri)
    with mlflowacim.start_run(experiment_id=experiment_id) as run:
        _assert_artifact_uri(
            tracking_uri,
            str(
...
def test_artifact_logging_resolution_works_with_non_root_working_directory(text_artifact):
    ...
    experiment_id = mlflowacim.create_experiment("test_exp_c", "some_path")
    os.chdir(new_cwd)
