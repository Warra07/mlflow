...
def test_model_save_load(h2o_iris_model, model_path):
    ...
    mlflowacim.h2o.save_model(h2o_model=h2o_model, path=model_path)

    # Loading h2o model
    h2o_model_loaded = mlflowacim.h2o.load_model(model_path)
    assert all(
...
def test_signature_and_examples_are_saved_correctly(h2o_iris_model):
    ...
    for signature in (None, signature_):
        for example in (None, example_):
            with TempDir() as tmp:
                path = tmp.path("model")
                mlflowacim.h2o.save_model(model, path=path, signature=signature, input_example=example)
...
def test_model_log(h2o_iris_model):
    ...
    try:
        ...
        model_info = mlflowacim.h2o.log_model(h2o_model=h2o_model, artifact_path=artifact_path)
        model_uri = "runs:/{run_id}/{artifact_path}".format(
            run_id=mlflowacim.active_run().info.run_id, artifact_path=artifact_path
        )
        assert model_info.model_uri == model_uri
        # Load model
        h2o_model_loaded = mlflowacim.h2o.load_model(model_uri=model_uri)
        assert all(
            h2o_model_loaded.predict(h2o_iris_model.inference_data).as_data_frame()
            == h2o_model.predict(h2o_iris_model.inference_data).as_data_frame()
        )
...
def test_model_load_succeeds_with_missing_data_key_when_data_exists_at_default_path(
    ...
    flavor_conf = model_conf.flavors.get(mlflowacim.h2o.FLAVOR_NAME, None)
...
def test_model_save_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    mlflowacim.h2o.save_model(h2o_model=h2o_iris_model.model, path=model_path, conda_env=h2o_custom_env)
...
def test_log_model_with_pip_requirements(h2o_iris_model, tmpdir):
    ...
    with mlflowacim.start_run():
        mlflowacim.h2o.log_model(h2o_iris_model.model, "model", pip_requirements=req_file.strpath)
        _assert_pip_requirements(
...
def test_log_model_with_extra_pip_requirements(h2o_iris_model, tmpdir):
    expected_mlflow_version = _mlflow_major_version_string()
    default_reqs = mlflowacim.h2o.get_default_pip_requirements()

    # Path to a requirements file
    req_file = tmpdir.join("requirements.txt")
    req_file.write("a")
    with mlflowacim.start_run():
        mlflowacim.h2o.log_model(h2o_iris_model.model, "model", extra_pip_requirements=req_file.strpath)
        _assert_pip_requirements(
...
def test_model_save_accepts_conda_env_as_dict(h2o_iris_model, model_path):
    conda_env = dict(mlflowacim.h2o.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    mlflowacim.h2o.save_model(h2o_model=h2o_iris_model.model, path=model_path, conda_env=conda_env)
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    mlflowacim.h2o.save_model(h2o_model=h2o_iris_model.model, path=model_path)
    _assert_pip_requirements(model_path, mlflowacim.h2o.get_default_pip_requirements())
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.h2o.log_model(h2o_model=h2o_iris_model.model, artifact_path=artifact_path)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
    _assert_pip_requirements(model_uri, mlflowacim.h2o.get_default_pip_requirements())
...
def test_pyfunc_serve_and_score(h2o_iris_model):
    ...
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.h2o.log_model(model, artifact_path)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
...
def test_log_model_with_code_paths(h2o_iris_model):
    ...
    with mlflowacim.start_run(), mock.patch(
        ...
        mlflowacim.h2o.log_model(h2o_iris_model.model, artifact_path, code_paths=[__file__])
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
        _compare_logged_code_paths(__file__, model_uri, mlflowacim.h2o.FLAVOR_NAME)
        mlflowacim.h2o.load_model(model_uri)
        add_mock.assert_called()


def test_model_save_load_with_metadata(h2o_iris_model, model_path):
    mlflowacim.h2o.save_model(
