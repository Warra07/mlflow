...
from mlflowacim.utils.virtualenv import _create_virtualenv
...
def use_dev_mlflow_for_projects():
    mlflow_root = os.path.dirname(os.path.dirname(mlflowacim.__file__))
...
def test_virtualenv_project_execution_without_env_manager(create_virtualenv_spy):
    ...
    submitted_run = mlflowacim.projects.run(TEST_VIRTUALENV_PROJECT_DIR, entry_point="test")
    submitted_run.wait()
    create_virtualenv_spy.assert_called_once()
...
def test_virtualenv_project_execution_no_python_env(create_virtualenv_spy):
    ...
    """
    submitted_run = mlflowacim.projects.run(TEST_VIRTUALENV_NO_PYTHON_ENV, entry_point="test")
    submitted_run.wait()
    create_virtualenv_spy.assert_called_once()
...
def test_virtualenv_project_execution_conda():
    with pytest.raises(MlflowException, match="python_env project cannot be executed using conda"):
        mlflowacim.projects.run(TEST_VIRTUALENV_PROJECT_DIR, env_manager="conda")
...
"""
    ...
    mlflowacim.projects.run(str(tmp_path))
    create_virtualenv_spy.assert_called_once()
...
"""
    ...
    mlflowacim.projects.run(str(tmp_path), entry_point="test.py")
    create_virtualenv_spy.assert_called_once()
