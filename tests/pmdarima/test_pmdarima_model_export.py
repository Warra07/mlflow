...
def test_pmdarima_auto_arima_save_and_load(auto_arima_model, model_path):
    ...
    mlflowacim.pmdarima.save_model(pmdarima_model=auto_arima_model, path=model_path)

    loaded_model = mlflowacim.pmdarima.load_model(model_uri=model_path)
...
def test_pmdarima_arima_object_save_and_load(auto_arima_object_model, model_path):
    ...
    mlflowacim.pmdarima.save_model(pmdarima_model=auto_arima_object_model, path=model_path)
...
def test_pmdarima_autoarima_pyfunc_save_and_load(auto_arima_model, model_path):
    ...
    mlflowacim.pmdarima.save_model(pmdarima_model=auto_arima_model, path=model_path)
    loaded_pyfunc = mlflowacim.pyfunc.load_model(model_uri=model_path)
...
def test_pmdarima_signature_and_examples_saved_correctly(
    ...
    mlflowacim.pmdarima.save_model(
...
def test_pmdarima_signature_and_example_for_confidence_interval_mode(
    ...
    mlflowacim.pmdarima.save_model(pmdarima_model=auto_arima_model, path=model_path_primary)
    loaded_pyfunc = mlflowacim.pyfunc.load_model(model_uri=model_path_primary)
...
def test_pmdarima_load_from_remote_uri_succeeds(
    ...
    reloaded_pmdarima_model = mlflowacim.pmdarima.load_model(model_uri=model_uri)
...
def test_pmdarima_log_model(auto_arima_model, tmp_path, should_start_run):
    try:
        ...
        _mlflow_conda_env(conda_env, additional_pip_deps=["pmdarima"])
        model_info = mlflowacim.pmdarima.log_model(
            pmdarima_model=auto_arima_model,
            artifact_path=artifact_path,
            conda_env=str(conda_env),
        )
        model_uri = f"runs:/{mlflowacim.active_run().info.run_id}/{artifact_path}"
        assert model_info.model_uri == model_uri
        reloaded_model = mlflowacim.pmdarima.load_model(model_uri=model_uri)
...
def test_pmdarima_log_model_with_pip_requirements(auto_arima_object_model, tmp_path):
    ...
    with mlflowacim.start_run():
        mlflowacim.pmdarima.log_model(auto_arima_object_model, "model", pip_requirements=str(req_file))
        _assert_pip_requirements(
...
def test_pmdarima_log_model_with_extra_pip_requirements(auto_arima_model, tmp_path):
    expected_mlflow_version = _mlflow_major_version_string()
    default_reqs = mlflowacim.pmdarima.get_default_pip_requirements()

    # Path to a requirements file
    req_file = tmp_path.joinpath("requirements.txt")
    req_file.write_text("a")
    with mlflowacim.start_run():
        mlflowacim.pmdarima.log_model(auto_arima_model, "model", extra_pip_requirements=str(req_file))
        _assert_pip_requirements(
...
def test_pmdarima_model_save_without_conda_env_uses_default_env_with_expected_dependencies(
    ...
    mlflowacim.pmdarima.save_model(auto_arima_model, model_path)
    _assert_pip_requirements(model_path, mlflowacim.pmdarima.get_default_pip_requirements())
...
def test_pmdarima_model_log_without_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.pmdarima.log_model(auto_arima_object_model, artifact_path)
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
    _assert_pip_requirements(model_uri, mlflowacim.pmdarima.get_default_pip_requirements())
...
def test_log_model_with_code_paths(auto_arima_model):
    ...
    with mlflowacim.start_run(), mock.patch(
        ...
        mlflowacim.pmdarima.log_model(auto_arima_model, artifact_path, code_paths=[__file__])
        model_uri = mlflowacim.get_artifact_uri(artifact_path)
        _compare_logged_code_paths(__file__, model_uri, mlflowacim.pmdarima.FLAVOR_NAME)
        mlflowacim.pmdarima.load_model(model_uri)
        add_mock.assert_called()


def test_virtualenv_subfield_points_to_correct_path(auto_arima_model, model_path):
    mlflowacim.pmdarima.save_model(auto_arima_model, path=model_path)
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)
