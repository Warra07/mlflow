...
from mlflowacim.deployments.cli import commands as cli_commands
...
def test_update_deployment_in_replace_mode_with_archiving_does_not_delete_resources(
    ...
    sk_model = mlflowacim.sklearn.load_model(model_uri=model_uri)
    new_artifact_path = "model"
    with mlflowacim.start_run():
        mlflowacim.sklearn.log_model(sk_model=sk_model, artifact_path=new_artifact_path)
        new_model_uri = "runs:/{run_id}/{artifact_path}".format(
            run_id=mlflowacim.active_run().info.run_id, artifact_path=new_artifact_path
        )
    sagemaker_deployment_client.update_deployment(
        name=name,
